...
```sql
saveMenu->setStartSlotCallback(<FileRef file-url="https://github.com/lilong555/Adventure-King/blob/60df0f40/this" undefined  file-path="this">Hii</FileRef> {
    // Bind this slot for the entire session
    saveManager->setActiveSaveSlot(slotIndex);
    
    if (!hasSave) {
        // New game: show role select
    } else {
        // Load game: populate runtime cache
    }
});
```
...
_uiController->init(
    ...
    <FileRef file-url="https://github.com/lilong555/Adventure-King/blob/60df0f40/this" undefined  file-path="this">Hii</FileRef> { return _levelMap && _player && _levelMap->isPointAtGate(_player->getPosition()); },
