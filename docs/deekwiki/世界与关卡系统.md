# World and Level Systems（世界与关卡系统）
...
TMX["TMXTiledMap<br>(*.tmx file)"]
CollisionGroup["'collision' Group<br>Rect/Polygon Objects"]
BornGroup["'born' Group<br>PlayerSpawn Object"]
GateGroup["'gate' Group<br>Rect Objects"]
EnemyGroup["'enemy_g' Group<br>Monster Spawn Points"]
ArenaGroup["'ArenaLayer' Group<br>ArenaTrigger/ArenaSpawn/ArenaGate"]
LoadFunc["load(gameLayer, tmxPath)"]
CreateCollision["createCollisionBodiesFromTMX('collision')"]
GetSpawn["getPlayerSpawnPoint('born')"]
LoadGates["loadGateAreas('gate')"]
LoadSpawns["loadEnemySpawnPoints('enemy_g')"]
LoadArenas["loadArenas('ArenaLayer')"]
TileMap["_tileMap<br>TMXTiledMap*"]
GateAreas["_gateAreas<br>vector"]
SpawnPoints["_enemySpawnPoints<br>vector"]
Arenas["_arenas<br>map"]
SpawnPos["position: Vec2"]
SpawnType["monsterType: string"]
SpawnCount["count: int"]
SpawnFlag["hasSpawned: bool"]
ArenaID["arenaID: string"]
TriggerRect["triggerRect: Rect"]
SpawnPts["spawnPoints: vector"]
Waves["waves: vector"]
Gates["gates: vector"]
ActiveCount["activeMonstersCount: int"]
WaveIndex["currentWaveIndex: int"]
Flags["isAct…16984 chars truncated…oad(isLevelCleared)"
  note over p3: "Set _isLevelCleared<br/>If true: activateGateVfxForRects()"
```
...
void LevelMap::registerRestoredArenaMonster(const std::string &arenaID, MonsterBase *monster, ...) {
    ...
    monster->setOnDeathCallback(<FileRef file-url="https://github.com/lilong555/Adventure-King/blob/60df0f40/this, arena, player, gameLayer, createMonsterByType" undefined  file-path="this, arena, player, gameLayer, createMonsterByType">Hii</FileRef> {
        arena->activeMonstersCount--;
        if (arena->activeMonstersCount <= 0) {
            arena->currentWaveIndex++;
            auto delay = DelayTime::create(1.5f);
            auto next = CallFunc::create([this, arena, ...] {
