...
> 说明：本节原文在仓库导出时被截断，以下内容保留原样的截断占位，避免信息丢失。

系统…（原文此处被截断：`…3049 chars truncated…`）… [Adventure-King/Classes/Character/Player/PlayerCharacter.cpp L451-L512](https://github.com/lilong555/Adventure-King/blob/60df0f40/Adventure-King/Classes/Character/Player/PlayerCharacter.cpp#L451-L512)
...
> 说明：原文此处是一个空的代码块（可能用于后续插入状态机图），这里按原样保留。
...
| `Game…1034 chars truncated…` | 技能点数分配规则 | 限制主动/被动点数，防止玩家在单一类别过度投入，促使角色成长更均衡。 |

> 说明：上表第 2 行原文被截断，保留原样的 `…1034 chars truncated…` 占位以避免遗漏。
