...
> 说明：上方 mermaid 中包含生成器截断占位 `…421 chars truncated…`，已原样保留以避免遗漏。
...
sequenceDiagram
  ...
  p4->>p6: MysteryForestScene…734 chars truncated…
```

> 说明：上方 sequenceDiagram 在原文中存在截断占位 `…734 chars truncated…`，已原样保留以避免遗漏。
