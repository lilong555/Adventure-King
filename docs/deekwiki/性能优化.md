...
1. **更新调用分布**：记录 `updateAI()` 执行时间戳，用于验证是否做到错帧分布
2. **活跃/非活跃比例**：按帧统计 `isWithinActiveUpdateRange()` 的判定结果
3. **帧时间拆分**：对尖刺帧做 Profiling，定位未被节流的高开销操作
...
在为新的怪物类型接入性能优化时，建议检查：

* 在 `setupRegistry()` 中调用 `preloadResources()` 静态方法
* 使用默认节流间隔，或通过 `setUpdateTickIntervals()` 自定义
* 确保高开销逻辑（寻路、可见性检查等）放在 `updateAI()`，而不是 `update()`
* 同时生成 20+ 只怪物进行压测，验证是否正确错帧
* 对比“全部活跃”与“多数非活跃”的帧时间，衡量基于距离裁剪的收益

**来源：** [Adventure-King/Classes/Character/Monster/Monsters/GoblinMonster.cpp L168-L179](https://github.com/lilong555/Adventure-King/blob/60df0f40/Adventure-King/Classes/Character/Monster/Monsters/GoblinMonster.cpp#L168-L179)

 [Adventure-King/Classes/Character/Monster/MonsterBase.cpp L426-L437](https://github.com/lilong555/Adventure-King/blob/60df0f40/Adventure-King/Classes/Character/Monster/MonsterBase.cpp#L426-L437)
