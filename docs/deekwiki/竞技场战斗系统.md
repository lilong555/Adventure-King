...
| 字段 | 类型 | 用途 |
| --- | --- | --- |
| `arenaID` | `std::string` | 唯一标识（来自 TMX 对象 `arenaID` 属性） |
| `triggerRect` | `cocos2d::Rect` | 激活区域（检测玩家进入） |
| `spawnPoints` | `std::vector<cocos2d::Vec2>` | 竞技场刷怪点（从 TMX 的 `ArenaSpawn` 对象收集） |
| `gates` | `std::vector<cocos2d::Node*>` | 门禁碰撞体（激活时启用，完成后禁用） |
| `waves` | `std::vector<WaveData>` | 波次定义（每波一个“怪物类型→数量”的映射） |
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| `enemies` | `std::map<std::string, int>` | 本波怪物组成：`type -> count`（例如 `"goblin" -> 3`） |

**来源**：[Adventure-King/Classes/Configs/ArenaConfig.h L7-L23](https://github.com/lilong555/Adventure-King/blob/60df0f40/Adventure-King/Classes/Configs/ArenaConfig.h#L7-L23)

 [Adventure-King/Classes/Scenes/LevelMap.cpp L959-L1120](https://github.com/lilong555/Adventure-King/blob/60df0f40/Adventure-King/Classes/Scenes/LevelMap.cpp#L959-L1120)
