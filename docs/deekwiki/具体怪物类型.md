...
ensureLoopAnimationCached(
    "goblin_walk",                                    // Cache…2931 chars truncated…LBACK_TIME (1.0f)
```

> 说明：上述 YAML 片段包含原文导出时的截断占位 `Cache…2931 chars truncated…`，按原样保留以避免遗漏。

---

## 攻击实现（Goblin）

> 说明：本节以下内容来自原文在导出时截断后的残留片段，包含完整的 mermaid 流程与代码块。为避免遗漏，保留原始结构，并对说明文字做中文化处理。
