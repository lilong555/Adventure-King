...
sequenceDiagram
  ...
  participant p4 as PlayerDeathMenu

  p1->>p2: isDead()?
  p2-->>p1: true
  p1->>p3: isDeathMenuShowing()?
  p3-->>p1: false
  p1->>p3: showDeathMenu()
  p3->>p4: create and display
  p4->>p4: Freeze input<br/>Show restart/return options
  p1->>p3: update(dt)
  p3->>p3: U…2022 chars truncated…as _boss (MonsterBase*)
  participant p5 as LevelMap
  participant p6 as SaveManager

  note over p1: _isPaused = false<br/>_player->isDead() = false
  p1->>p2: update(dt)
  note over p2: Process movement intent<br/>Cooldown timers<br/>Ground contact state
  p1->>p3: update(dt)
  note over p3: Update HUD elements<br/>HP/MP/XP bars<br/>Skill cooldowns
  p1->>p4: isDead()?
  alt Boss is dead
    p1->>p3: getGameUI()->unbindBoss()
    p1->>p1: _boss = nullptr
  end
  p1->>p5: updateEnemySpawns(player, gameLayer, createMonsterByType, viewDistance, dt)
  note over p5: Check spawn proximity<br/>Throttled update (every 0.2s)
  p1->>p5: updateArenas(player, gameLayer, createMonsterByType)
  note over p5: Check arena triggers<br/>Spawn wave monsters
  p1->>p1: processSaveRequests(dt)
  p1->>p6: consumeImmediateSaveRequest()?
  alt Immediate save pending
    p1->>p1: saveToActiveSlotInternal()
    p1->>p3: showToast("自动保存成功")
  else Auto-save timer elapsed
    p1->>p6: tickAutoSave(dt)?
    p1->>p1: saveToActiveSlotInternal()
    p1->>p3: showToast("自动保存")
  end
```

> 说明：原文该段中包含一处由生成器截断的占位文本 `U…2022 chars truncated…`，为避免遗漏已原样保留。
