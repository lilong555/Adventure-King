# 关键概念与术语（Key Concepts and Terminology）
...
| 职责 | 相关接口/字段 |
| --- | --- |
| 组件访问（原文截断：`T…460 chars truncated…`） | `getAttributeComponent()`, `getSkillComponent()`, `getStateMachineComponent()` |
...
> 说明：原文在此处包含一段被生成器截断的占位文本（为避免遗漏，原样保留）：
> `TryUseSki…981 chars truncated…`
...
PlayerCharacter --> TryNormalAttack
PlayerCharacter --> TryUseSkill
```
