...
> 说明：本节原始英文文档中间存在生成器截断占位（`...18037 chars truncated...`）以及不完整片段；为保证“无遗漏”，下方保留原占位与残片内容。

伤害计算发生在 `CharacterBase::takeDamage()` 中，并遵循严格顺序以确保结果一致。……（原文此处被截断：`…18037 chars truncated…`）……为避免在物理回调期间修改场景树，伤害应用会延迟到下一帧执行：
...
if (!monster->isScheduled(key))
{
    monster->scheduleOnce(
        <FileRef file-url="https://github.com/lilong555/Adventure-King/blob/60df0f40/monster, dmg" undefined  file-path="monster, dmg">Hii</FileRef>
        {
...
`isScheduled()` 的检查用于避免重复回调：如果物理引擎对同一对接触体触发了多次 begin 事件，仍然只会安排一次伤害结算。
